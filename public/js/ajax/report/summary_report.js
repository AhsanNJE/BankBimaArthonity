$(document).ready(function () {
    ...
    $(document).on('change', '#startDate, #endDate', function(e){
        ...
        searchTransaction(`/summary/report/search`, {startDate:startDate, endDate:endDate})
    });
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.status === 'success') {
                    $('.summary-report').html(res.data);
                    if (res.paginate) {
                        $('.summary-report').append('<div class="center search-paginate" id="paginate">' + res.paginate + '</div>');
                    }
                }
                else {
                    $('.summary-report').html(`<span class="text-danger">Result not Found </span>`);
                }
            }
        });
    }
